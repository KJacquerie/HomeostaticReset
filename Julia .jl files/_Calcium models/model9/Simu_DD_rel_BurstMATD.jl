...
# Include model
include("model_DD_rel_BurstMATD.jl")
...
function simu_wI()
    for j=1:1:length(w_init_vec)
        ...
        for p=1:1:length(IstepI_vec)
            ...
            cd(@sprintf("/Users/kathleen/Documents/PhD/2022-reset/model9/results_sleep/%s/w%d/%d/",strength,w_init_vec[j]*10,p))
...
VplotI = plot(
    t[Ttransient:Tdt],
    Vconnect[Ttransient:Tdt, nEcells+1:ncells-nCcells],
    color = "red",
    ylabel="I [mV]",
...
Vplot1 = plot(
    t[Ttransient:Tdt],
    Vconnect[Ttransient:Tdt, 1:nEcells],
    ylabel="E [mV]",
    legend= (frameon=false),
    color = "blue")
...
Vplot2 = plot(
    t[Ttransient:Tdt],
    Vconnect[Ttransient:Tdt, nEcells+nIcells+1:ncells],
    color = "grey",
    ylabel="C [mV]",
    legend= (frameon=false)
)
