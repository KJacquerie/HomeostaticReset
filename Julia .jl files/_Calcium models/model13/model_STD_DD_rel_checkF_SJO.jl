  ...
  for z = 1:Tdt
    ...
    for j = 1:ncells
      ...
      if (j<=nEcells)
        ...
        if count_break_pre<5 &&freq!=10000 && go_pre==1
          if (mod(z_delayed, freq) > freq - duration && z_delayed >=0 )
            Iappstep = IstepE
          else
            Iappstep = 0.
          end
        elseif(mod(idx_break_pre,1e4/dt)==0)
          count_break_pre=0
          idx_break_pre=0
          if(delay_pre<0)
            go_pre=0
      ...
      if ( j > nIcells+nEcells && j <=ncells)
        ...
        if count_break_post <5 && freq!=10000
          if (mod(z_delayed, freq) > freq - duration && z_delayed >=0 )
            Iappstep = IstepC
          else
            Iappstep = 0.
          end
        elseif(mod(idx_break_post,1e4/dt)==0)
          count_break_post=0
          idx_break_post=0
          if(delay_pre<0)
            go_pre=0
    ...
    cpre_var  += (dt)*(1)*(-cpre_prev/tau_Ca) +event_pre*C_Pre*ww_var*xres*U
    cpost_var += (dt)*(1)*(-cpost_prev/tau_Ca)+event_post*C_Post#+ eta_lin*event_post*cpre_prev
    xres += (dt)*((1-xres)/tau_rec) - U*xres*event_pre
    ca_var = cpre_prev +cpost_prev
    #ww_var += (dt)*(1/tau_w)*(gamma_p*(1-ww_var)*Heaviside_p(ca_var,theta_p) - gamma_d*ww_var*Heaviside_d(ca_var, theta_d, theta_p) )


    if(ca_var>= theta_p)
      ww_var +=(dt)*(1/tauw_p)*(Omega_p)
    elseif(ca_var>=theta_d && ca_var<theta_p)
      ww_var +=(dt)*(1/tauw_d)*(Omega_d)
    else
      ww_var +=(dt)*0.
    end
    if (ww_var>=wMAX)
    ...
    cpre_prev   = copy(cpre_var)
    cpost_prev  = copy(cpost_var)
    #c_plot[z]  = cpre_var + cpost_var#cpre[z] + cpost[z]
    ww[z] = copy(ww_var)
    #VV[z,:] = copy(V')
