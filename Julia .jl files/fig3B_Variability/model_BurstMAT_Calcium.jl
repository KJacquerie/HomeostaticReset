...
function simulateTOY_ncells(ncells::Int64,nEcells::Int64,nIcells::Int64,IappE::Float64,IappI::Float64,TstepEinit::Int64,TstepEfinal::Int64,IstepE::Float64,TstepIinit1::Int64,TstepIinit2::Int64,TstepIinit3::Int64,TstepIinit4::Int64,TstepIfinal::Int64,IstepI1::Float64,IstepI2::Float64,IstepI3::Float64,IstepI4::Float64, gEC::Float64, freq::Float64, delay_pre::Float64, w_init::Float64, j_net::Int64)
  ...
  Ca = ones(ncells)
  Ca[1]=((-k1_E/k2_E)*ICaT(V[1],mCaT[1],hCaT[1],gCaT_E))
  Ca[2]=((-k1_E/k2_E)*ICaT(V[1],mCaT[1],hCaT[1],gCaT_I))
  Ca[3]=((-k1_E/k2_E)*ICaT(V[1],mCaT[1],hCaT[1],gCaT_C))
  AMPA=zeros(ncells)
  GABAA = zeros(ncells)
  GABAB = zeros(ncells)
  sSYN = zeros(ncells)
  ...
  for z = 1:Tdt
    ...
    for j = 1:ncells
      ...
      if j==1#(j<=nEcells)
        ...
        V[j] += dV(gNavec_E[j_net], gKdvec_E[j_net], glvec_E[j_net], V[j], mNa[j], hNa[j], mKd[j], mCaT[j], hCaT[j], mH[j], Ca[j], Iapp, Iappstep, gCaTvec_E[j_net], gHvec_E[j_net], gKCavec_E[j_net])
        Ca[j] += dCa(Vprev[j],mCaT[j],hCaT[j],Ca[j],gCaTvec_E[j_net],k1vec_E[j_net], k2vec_E[j_net])
      ...
      if j==2 #(j > nEcells && j <= nEcells+nIcells)
        ...
        V[j] += dV(gNavec_I[j_net], gKdvec_I[j_net], glvec_I[j_net], V[j], mNa[j], hNa[j], mKd[j], mCaT[j], hCaT[j], mH[j], Ca[j], Iapp, Iappstep, gCaTvec_I[j_net], gHvec_I[j_net], gKCavec_I[j_net])
        Ca[j] += dCa(Vprev[j],mCaT[j],hCaT[j],Ca[j],gCaTvec_I[j_net],k1vec_I[j_net], k2vec_I[j_net])
      ...
      if j==3#( j > nIcells+nEcells && j <=ncells)
        ...
        V[j] += dV(gNavec_C[j_net], gKdvec_C[j_net], glvec_C[j_net], V[j], mNa[j], hNa[j], mKd[j], mCaT[j], hCaT[j], mH[j], Ca[j], Iapp, Iappstep, gCaTvec_C[j_net], gHvec_C[j_net], gKCavec_C[j_net])
        Ca[j] += dCa(Vprev[j],mCaT[j],hCaT[j],Ca[j],gCaTvec_C[j_net],k1vec_C[j_net], k2vec_C[j_net])
      end
