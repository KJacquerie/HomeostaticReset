...
# Include model
include("model_BurstMAT_Calcium.jl")
...
function simu_wI()
    for idx_expm=1:1:length(gEC_mat)
        if idx_expm==1
            strength ="weak"
        else
            strength ="strong"
        ...
        for j=1:1:length(w_init_vec)
            ...
            for p=1:1:length(IstepI_vec)
                ...
                cd(@sprintf("/Users/JuliettePonnet/Documents/MASTER2/TFE/Code/Graupner2016/GB_DD/results/results_%s/netBurstMATD/",expm))
                if !in.(strength,Ref(readdir()))
                    mkdir(strength)
                    mkdir(@sprintf("%s/w%d/", strength,w_init_vec[j]*10))
                    mkdir(@sprintf("%s/w%d/%d/", strength,w_init_vec[j]*10,p))
                elseif !in.(@sprintf("w%d",w_init_vec[j]*10),Ref(readdir(strength)))
                    mkdir(@sprintf("%s/w%d/", strength,w_init_vec[j]*10))
                    mkdir(@sprintf("%s/w%d/%d/", strength,w_init_vec[j]*10,p))
                elseif !in.(@sprintf("%d",p),Ref(readdir(@sprintf("%s/w%d/", strength,w_init_vec[j]*10))))
                    mkdir(@sprintf("%s/w%d/%d/", strength,w_init_vec[j]*10,p))
                end
                cd(@sprintf("/Users/JuliettePonnet/Documents/MASTER2/TFE/Code/Graupner2016/GB_DD/results/results_%s/netBurstMATD/%s/w%d/%d/",expm,strength,w_init_vec[j]*10,p))
