...
directory_name = "/Users/kathleen/Documents/PhD/2022-reset/LFP"
# Loads packages
using Plots
...
const Omega_p_sleep = 0.7#gamma_p/(gamma_p+gamma_d)
const Omega_d = 0.
const Omega_0 = 0.
...
@time (w,V, LFP_E, LFP_I, LFP_C) = simulateTOY_ncellsScenarioNMOD(
    ncells,
    ncellsI,
    ncellsC,
    Iapp_cell,
...
LFP_E_filt = filt(digitalfilter(responsetype, designmethod), LFP_E)
LFP_I_filt = filt(digitalfilter(responsetype, designmethod), LFP_I)
...
writedlm(@sprintf("%s/results_LFP/LFP_E.dat", directory_name), LFP_E_filt, header = false)
writedlm(@sprintf("%s/results_LFP/LFP_I.dat", directory_name),LFP_I_filt, header = false)
writedlm(@sprintf("%s/results_LFP/LFP_C.dat", directory_name),LFP_C_filt, header = false)
writedlm(@sprintf("%s/results_LFP/V.dat", directory_name),V, header = false)
writedlm(@sprintf("%s/results_LFP/w.dat", directory_name),w, header = false)
