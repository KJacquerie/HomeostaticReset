...
const gNavec_C = gNa_C * ones(ncells) #*(1-2*variab*(rand(1)[1]-0.5))#- 70*(rand(ncells)-0.5)
const gKdvec_C = gKd_C * ones(ncells) #*(1-2*variab*(rand(1)[1]-0.5)) #- 40*(rand(ncells)-0.5)
const gHvec_C = gH_C * ones(ncells) #*(1-2*variab*(rand(1)[1]-0.5)) #- 0.002*(rand(ncells)-0.5)
const gCaTvec_C = gCaT_C * ones(ncells)#*(1-2*variab*(rand(1)[1]-0.5)) #- 0.35*(rand(ncells)-0.5)
const gKCavec_C = gKCa_C * ones(ncells) #*(1-2*variab*(rand(1)[1]-0.5))#- 2*(rand(ncells)-0.5)
const glvec_C = gl_C * ones(ncells) #*(1-2*variab*(rand(1)[1]-0.5))#- 0.015*(rand(ncells)-0.5)
const k1vec_C = k1_C * ones(ncells) #- 0.05*(rand(ncells))
const k2vec_C = k2_C * ones(ncells) #- 0.005*(rand(ncells))
...
#STDP Bi and Poo
const Am_init = 0.0053
const Ap_init = 0.0096
const tau_m = 33.7
const tau_p = 16.8
SB = 1
...
function simu_wI()
        ...
        for j=1:1:length(w_init_vec)
            ...
            for p=1:1:length(IstepI_vec)
                ...
                cd(@sprintf("/Users/kathleen/Documents/PhD/2022-reset/NMOD-SP/phenom/Ampl_TAG/results_%s/w%d/%d/",expm,w_init_vec[j]*10,p))
