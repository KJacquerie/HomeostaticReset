...
function simulate_SJO_Triplet(freq::Float64, delay_pre::Float64, w::Float64, SB::Int64)
  gEC = gEC_const*w
  # Initial conditions
  V=-60*ones(ncells)
...
## plasticity
      ...
      elseif last == 0
        ...
        if SB == 0
          new = x_prev*(A2_p+ A3_p*y2_prev) + w
        else
    ...
    if delay_pre >= 0 && count_post == count_f
      if (count_post == count_f)
        stop = z
        break
      end
    end
    if delay_pre <= 0 && count_pre == count_f
...
if ( j > 1+nIcells && j <=ncells)
  ...
  if (z >= tStartC/dt && z< tStopC/dt || burst == 1)
    Iappstep = IstepC2
  end
